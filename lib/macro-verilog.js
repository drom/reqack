...
const forkCtrl = p => {
    ...
    const locals = (p.targets.length > 1) ? 'logic ' + p.targets.map((e, ei) =>
        `ack${p.id}_${ei}_r`
    ).join(', ') + ';' : '';
    ...
    const acks = (p.targets.length > 1) ? p.targets.map((e, ei) =>
        `logic ack${p.id}s; assign ack${p.id}s = ack${p.id}_${ei} | ~req${p.id}_${ei};`
    ).join('\n') : '';
    ...
    const ackr = (p.targets.length > 1) ? p.targets.map((e, ei) =>
        `always_ff @(posedge clk or negedge reset_n) if (~reset_n) ack${p.id}_${ei}_r <= 1'b0; else ack${p.id}_${ei}_r <= ack${p.id}_${ei} & ~ack${p.id}m;`
    ).join('\n') : '';
