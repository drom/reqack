        ...
        res
            ...
            .concat(assign(
                ...
                e.targets.map((target, ni) =>
                    ackPrefix(ei + '_' + ni)).join(' & ')
            ))
            .concat(e.targets.map((target, ni) =>
                'always_ff @(posedge clk or negedge reset_n) if (~reset_n) ' +
                ackPrefix(ei + '_' + ni + '_r') + ' <= 1\'b0; else ' +
                ackPrefix(ei + '_' + ni + '_r') + ' <= ' +
                ackPrefix(ei + '_' + ni) + ' & ~' +
                ackPrefix(ei) + ';'))
            .concat(e.targets.map((target, ni) =>
                assign(
                    reqPrefix(ei + '_' + ni),
                    reqPrefix(ei) + ' & ~' + ackPrefix(ei + '_' + ni + '_r')
                ))
            )
            .concat((e.label.capacity === 1) ?
                assign(enPrefix(ei), reqPrefix(ei) + ' & ' + ackPrefix(ei)) :
                []
            )
    ), []);
