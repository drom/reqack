...
`;
    ...
    const standardDataPathInstance = (label, macros) => {
        ...
        return p => {
            ...
            const targets = p.t.map((sig, sigi) => [datSuffix('t_' + sigi), sig.name]);
            const initiators = p.i.map((sig, sigi) => [datSuffix('i_' + sigi), sig.name]);
            const ports = targets
                .concat(initiators)
                .concat(extra)
    ...
    const cJoins = () =>
        g.nodes.reduce((res, n, ni) => {
            if (n.from.length > 0 && n.to.length > 0) { // not IO node
                ...
                if (
                    macros[label] && (macros[label].ctrl2data || (n.to.length > 1))
                ) { // need custom controller
                    ...
                    const bindTargets = n.from.reduce((eres, e, ei) => {
                        ...
                        return eres.concat([
                            [reqSuffix('t_' + ei), reqPrefix(ename)],
                            [ackSuffix('t_' + ei), ackPrefix(ename)]
                        ]);
                    }, []);

                    const bindInitiators = n.to.reduce((eres, e, ei) => eres.concat([
                        [reqSuffix('i_' + ei), reqPrefix(findGlobalIndexOfEdge(g, e))],
                        [ackSuffix('i_' + ei), ackPrefix(findGlobalIndexOfEdge(g, e))]
                    ]), []);
