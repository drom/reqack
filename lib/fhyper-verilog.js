...
const dff = g => ['// per edge']
    .concat(
        g.edges.reduce(function (eres, e, ei) {
            ...
            if (e.label.capacity == 1) {
                return eres.concat([
                    'reg' + vectorDim(e.label.width) + edgeNname + '_r;',
                    'always @(posedge clk) if (' + enPrefix(ei) + ') ' + edgeNname + '_r <= ' + edgeNname + '_nxt;',
                    assign(edgeNname, edgeNname + '_r'),
                    ''
                ]);
            } else
            if (e.label.capacity == 1.5) {
                return eres.concat([
                    'reg' + vectorDim(e.label.width) + edgeNname + '_r0, ' + edgeNname + '_r1;',
                    'always @(posedge clk) if (' + enPrefix(ei) + '_0) ' + edgeNname + '_r0 <= ' + edgeNname + '_nxt;',
                    'always @(posedge clk) if (' + enPrefix(ei) + '_1) ' + edgeNname + '_r1 <= ' + edgeNname + '_nxt;',
