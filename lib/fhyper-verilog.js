    ...
    return p => {
      const paramInterface = parameters ? (
        ' #(\n' + Object.keys(parameters)
          .map(param => indent + '.' + param + '(NODE' + p.id + '_' + param + ')')
          .join(',\n') + '\n)'
      ) : '';
      const ctrl2dataWires = ctrl2data(p);
      const extra = Object.keys(ctrl2dataWires).map(sig => [ctrl2dataWires[sig][0], ctrl2dataWires[sig][1]]);
